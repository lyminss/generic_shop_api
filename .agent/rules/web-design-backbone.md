...
UI phải phục vụ user flow, không chỉ phục vụ visual.
Mỗi page phải xác định rõ:
...
- Success state
- Failure state
- Không đặt quá nhiều CTA có cùng mức độ ưu tiên.
- Navigation phải predictable.
  Người dùng phải biết:
- đang ở đâu
- có thể làm gì
- thao tác vừa thực hiện thành công hay chưa
- tiếp theo nên làm gì
- Không tạo UI chỉ để “trang trí”.
  Ưu tiên:
- Clarity → Usability → Accessibility → Visual polish.
- Visual hierarchy phải dẫn mắt người dùng theo đúng business flow.

## 14. Design System Consistency

- KHÔNG được tự tạo style mới cho từng component nếu design system đã có.
- Không được có 5 kiểu button khác nhau nếu chúng cùng một semantic role.

## 15. Component State — Bắt buộc

- Mọi interactive component phải thiết kế đầy đủ state:
  Default
  Hover
  Focus
  Active
